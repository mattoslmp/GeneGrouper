...
## --------------Start script---------------------##
def OutputRegionResults(UserInput_main_dir, UserInput_output_dir_name):
    ...
    dend = shc.dendrogram(shc.linkage(y=df_jac, method='ward', optimal_ordering=True), labels=df_jac.columns)

    rep_label_map = pd.DataFrame({'representative_id': rep_list, 'dbscan_label': label_list})
    df_reorder = pd.DataFrame({'representative_id': dend['ivl'], 'order': range(0, len(dend['ivl']))})
    ...
    # Ensure 'dbscan_label' is numeric and not multi-dimensional
    df_t1['dbscan_label'] = pd.to_numeric(df_t1['dbscan_label'], errors='coerce')

    # Filter for numeric data only before calculating mean
    numeric_columns = df_t1.select_dtypes(include=[np.number]).columns.tolist()
    df_t1_numeric = df_t1[['dbscan_label', 'variable', 'value']]

    max_result_vals = df_t1_numeric.groupby(['dbscan_label', 'variable'])['value'].max().reset_index()
    ...
    min_result_vals = df_t1_numeric.groupby(['dbscan_label', 'variable'])['value'].min().reset_index()
    ...
    mean_result_vals = df_t1_numeric.groupby(['dbscan_label', 'variable'])['value'].mean().reset_index()
    ...
    sd_result_vals = df_t1_numeric.groupby(['dbscan_label', 'variable'])['value'].std().reset_index()
    ...
    df_r1 = df_r1[['dbscan_label', 'member_counts', 'min_mean_dissimilarity', 'mean_mean_dissimilarity', 'max_mean_dissimilarity', 'sd_mean_dissimilarity', 'min_pident', 'mean_pident', 'max_pident', 'sd_pident', 'min_qcovs', 'mean_qcovs', 'max_qcovs', 'sd_qcovs']]
    ...
    df_t1 = pd.read_csv(pjoin('internal_data', 'full_region.csv'))[['dbscan_label', 'region_id', 'assembly_id', 'label_representative', 'full_index_pos', 'refseq_locus_tag', 'refseq_gene', 'refseq_product', 'gene_position', 'is_seed']]
    ...
    df_l1 = df_l1[['region_id', 'dbscan_label', 'group_representative', 'assembly_id', 'contig_id', 'gene_position', 'orf_id', 'pseudo_check', 'refseq_locus_tag', 'refseq_gene', 'refseq_product', 'is_seed', 'pident', 'qcovs', 'evalue', 'ortho_cluster_id', 'ortho_cluster_members', 'mmseqs_cluster_id', 'mmseqs_cluster_representative', 'mean_dissimilarity', 'representative_relative_dissimilarity', 'organism', 'genus', 'species', 'cds_start', 'cds_end', 'strand', 'relative_seed_position', 'order']]
